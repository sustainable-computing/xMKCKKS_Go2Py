...
func genTestParams(defaultParam Parameters, idset *mkrlwe.IDSet) (testContext *testParams, err error) {
	...
	// newPkSet := mkrlwe.NewPublicKeyKeySet()

	// sumPk.PublicKey.Value = new()
	// counter := 0
	pk1 := testContext.pkSet.Value["user0"]
	pk2 := testContext.pkSet.Value["user1"]
	// pk3 := testContext.pkSet.Value["user2"]
	// pk1.PublicKey.Value[1].P.Coeffs = pk2.PublicKey.Value[1].P.Coeffs
	// pk1.PublicKey.Value[1].Q.Coeffs = pk2.PublicKey.Value[1].Q.Coeffs
	// ringQP := defaultParam.Parameters.RingQP()
	// levelQ, levelP := defaultParam.Parameters.QCount()-1, defaultParam.Parameters.PCount()-1
	levelP := defaultParam.Parameters.PCount() - 1

	// levelQ, levelP := defaultParam.Parameters.QCount(), defaultParam.Parameters.PCount()

	fmt.Print(reflect.DeepEqual(pk1.PublicKey.Value[0].P.Coeffs, pk2.PublicKey.Value[0].P.Coeffs))
	// fmt.Print(reflect.DeepEqual(pk2.PublicKey.Value[0].P.Coeffs, pk3.PublicKey.Value[0].P.Coeffs))

	fmt.Print(reflect.DeepEqual(pk1.PublicKey.Value[0].Q.Coeffs, pk2.PublicKey.Value[0].Q.Coeffs))
	// fmt.Print(reflect.DeepEqual(pk2.PublicKey.Value[0].Q.Coeffs, pk3.PublicKey.Value[0].Q.Coeffs))

	sumPk := pk1.CopyNew()

	// sumQP := new(rlwe.PolyQP)
	// ringQP.AddLvl(levelQ, levelP, pk1.PublicKey.Value[0], pk2.PublicKey.Value[0], sumPk.Value[0])
	ringP := defaultParam.RingP()
	ringP.AddLvl(levelP, pk1.PublicKey.Value[0].P, pk2.PublicKey.Value[0].P, sumPk.Value[0].P)
	// fmt.Print("Q pk1:", pk1.PublicKey.Value[0].Q, "\n")
	// fmt.Print("Q pk2:", pk2.PublicKey.Value[0].Q, "\n")

	fmt.Print("\nQ pk1:", len(pk1.PublicKey.Value[0].Q.Coeffs[0]), len(pk1.PublicKey.Value[0].Q.Coeffs), "\n")
	fmt.Print("\nQ pk2:", len(pk2.PublicKey.Value[0].Q.Coeffs[0]), len(pk2.PublicKey.Value[0].Q.Coeffs), "\n")

	// // fmt.Print("\n EQUALS_Q:", reflect.DeepEqual(pk1.PublicKey.Value[0].Q.Coeffs, pk2.PublicKey.Value[0].Q.Coeffs), "\n")
	// fmt.Print("\n EQUALS_Q:", reflect.DeepEqual(pk1.PublicKey.Value[0].Q.Coeffs, pk2.PublicKey.Value[0].Q.Coeffs), "\n")

	// ringQP.AddLvl(levelQ, levelP, sumPk.Value[0], pk3.PublicKey.Value[0], sumPk.Value[0])
	// ringQP.AddNoModLvl(levelQ, levelP, pk1.PublicKey.Value[0], pk2.PublicKey.Value[0], sumPk.Value[0])
	// ringQP.AddNoModLvl(levelQ, levelP, sumPk.Value[0], pk3.PublicKey.Value[0], sumPk.Value[0])

	// testContext.pkSet.Value["user0"].PublicKey.Value[0] = sumPk.Value[0].CopyNew()
	// testContext.pkSet.Value["user1"].PublicKey.Value[0] = sumPk.Value[0].CopyNew()

	testContext.pkSet.Value["user0"].PublicKey.Value[0].P = sumPk.Value[0].P.CopyNew()
	testContext.pkSet.Value["user1"].PublicKey.Value[0].P = sumPk.Value[0].P.CopyNew()

	// testContext.pkSet.Value["user1"].PublicKey.Value[0].P = testContext.pkSet.Value["user0"].PublicKey.Value[0].P.CopyNew()

	// testContext.pkSet.Value["user2"].PublicKey.Value[0] = sumPk.Value[0].CopyNew()

	fmt.Print("equals", reflect.DeepEqual(testContext.pkSet.Value["user0"].PublicKey.Value[0], testContext.pkSet.Value["user1"].PublicKey.Value[0]), "\n")
	// fmt.Print("equals", reflect.DeepEqual(testContext.pkSet.Value["user1"].PublicKey.Value[0], testContext.pkSet.Value["user2"].PublicKey.Value[0]))

	// fmt.Print(reflect.DeepEqual(pk1.PublicKey.Value[1].P.Coeffs, pk2.PublicKey.Value[1].P.Coeffs))
	// fmt.Print(reflect.DeepEqual(pk2.PublicKey.Value[1].P.Coeffs, pk3.PublicKey.Value[1].P.Coeffs))

	// fmt.Print(reflect.DeepEqual(pk1.PublicKey.Value[1].Q.Coeffs, pk2.PublicKey.Value[1].Q.Coeffs))
	// fmt.Print(reflect.DeepEqual(pk2.PublicKey.Value[1].Q.Coeffs, pk3.PublicKey.Value[1].Q.Coeffs))

	// for _, pk := range testContext.pkSet.Value {
	// 	fmt.Printf("pkid" + strconv.Itoa(counter))
	// 	fmt.Print(pk.Value[0].P.Coeffs == )
	// 	fmt.Print(pk.Value[1].Q.Coeffs)
	// 	// var format string = "pkid" + strconv.Itoa(counter) + pk.Value[0].P.Coeffs + pk.Value[1].Q.Coeffs
...
func testEvaluatorAdd(testContext *testParams, t *testing.T) {
	...
	t.Run(GetTestName(testContext.params, "Evaluator/Add/CtCt/"), func(t *testing.T) {
		params := testContext.params
		msg1, ct1 := newTestVectors(testContext, "user0", complex(-1, -1), complex(1, 1))
		msg2, ct2 := newTestVectors(testContext, "user1", complex(-1, -1), complex(1, 1))
